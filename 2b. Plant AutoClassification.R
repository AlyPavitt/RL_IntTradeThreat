...
######               2b. CLASSIFY THREAT FROM INT. TRADE                ######
####     FOR SUBSET OF PLANT/FUNGI SPECIES BASED ON IUCN RL ASSESSMENT    ####
...
rla <- read.csv(paste0(outputlocation, "1. RL_SpeciesShortlist.csv"))
...
# plants only, remove subspecies
rla <- rla[rla$kingdomName %in% c("PLANTAE", "FUNGI"),]
rla <- rla[!grepl(" ssp\\. |subsp\\. |var\\. ", rla$scientificName),]
...
# subset on coverage = All and P (plants) only to exclude animals
...
Trade_core <- Trade_core$String[Trade_core$Coverage %in% c("All", "P")]

Trade_prefix <- read.csv(paste0(librarytext, "Trade/Trade_prefix.csv"))
Trade_prefix <- Trade_prefix$String[Trade_prefix$Coverage %in% c("All", "P")]

Trade_suffix <- read.csv(paste0(librarytext, "Trade/Trade_suffix.csv"))
Trade_suffix <- Trade_suffix$String[Trade_suffix$Coverage %in% c("All", "P")]
...
NoTrade_core <- NoTrade_core$String[NoTrade_core$Coverage %in% c("All", "P")]
...
Int_TradeKeywords <- unique(c(paste(sapply(International_scope,paste,c("market","demand","touris.?.?",Trade_core))),
...
Threat_core <- c(paste0(sapply(c("over.?"),paste0,c(Trade_core))), "exploit.?.?.?.?.?.?")
Threat_TradeKeywordsPref <- c(Threat_core,"illegal.?.?.?trad", "illegal.?.?.?collect.", "vulnerab.?.?.?.?.?.?poach",paste(sapply(Threat_prefix,paste,c(Trade_core, Trade_suffix))))
...
## Remove threatcodes that are not relevant to plants ##

rla$ThreatCodes <- gsub(paste(c(threatcodes.animals),collapse="|"), "", rla$ThreatCodes)
...
sub$DT1[(grepl(paste(threatcodes.plants,collapse="|"),sub$ThreatCodes))] <- "1b.yes" # (iii)
...
# i. intentional large-scale use-related threat code (5.2.1, 5.3.1 and 5.3.2 for plants) selected (that is NOT coded up as "Future" or "Past unlikely to return") AND only international end use
# allowing for 5.2.1 only being found for non-timber plants and 5.3s for timber species
...
# (b) yes/probable
sub$DT4[grepl("5\\.2\\.1|5\\.3\\.1|5\\.3\\.2", sub$ThreatCodes) & !is.na(sub$International) & is.na(sub$Subsistence) & is.na(sub$National)] <- "4b.yes" #(i)
...
sub$DT4[is.na(sub$ThreatCodes) & (grepl(paste(threatcodes.plants,collapse="|"),sub$ThreatCodes_PastUnlikely))] <- "4a.no" #(ii)
...
# ii.Trade = threat in free text AND that kind of trade is in International_EndUses #(e.g. "horticulture is a threat" and Pets/display plants is in International_EndUses) (all species whether or not scored)
...
# Construction / household goods / handicrafts #
Construction <- c("timber", "logg", "fell")

Threat_TradeKeywords_Construction <- paste(c(Threat_TradeKeywordsPref[grepl(paste(Construction,collapse="|"),Threat_TradeKeywordsPref)],
                                     Threat_TradeKeywordsSuff[grepl(paste(Construction,collapse="|"),Threat_TradeKeywordsSuff)]))

TradeKeywords_Construction <- c(TradeKeywords[grepl(paste(Construction,collapse="|"),TradeKeywords)])  #deforestation


# relevant threat trade keywords in any of the free text fields AND international end use
rla$DT5[which(grepl(paste(Threat_TradeKeywords_Construction,collapse="|"),rla$cn) &
                  grepl("Construction|house|handicraft",rla$International_EndUses))] <- "5a.yes"


# relevant trade keywords in threat AND international end use
rla$DT5[which(grepl(paste(TradeKeywords_Construction,collapse="|"),rla$threats) &
                  grepl("Construction|house|handicraft",rla$International_EndUses))] <- "5a.yes"
...
# horticulture #
horticulture <- c("ornament", "plant", "seed", "poach", "collect", "succulent")

Threat_TradeKeywords_horticulture <- paste(c(Threat_TradeKeywordsPref[grepl(paste(horticulture,collapse="|"),Threat_TradeKeywordsPref)],
                                        Threat_TradeKeywordsSuff[grepl(paste(horticulture,collapse="|"),Threat_TradeKeywordsSuff)]))

TradeKeywords_horticulture <- c(TradeKeywords[grepl(paste(horticulture,collapse="|"),TradeKeywords)])


# relevant threat trade keywords in any of the free text fields AND international end use
rla$DT5[which(grepl(paste(Threat_TradeKeywords_horticulture,collapse="|"),rla$cn) &
                  grepl("horticulture",rla$International_EndUses))] <- "5a.yes"


# relevant trade keywords in threat AND international end use
rla$DT5[which(grepl(paste(TradeKeywords_horticulture,collapse="|"),rla$threats) &
                  grepl("horticulture",rla$International_EndUses))] <- "5a.yes"
...
write.csv(rla, paste0(outputlocation,"Allplants_", Sys.Date(), ".csv"), row.names = F)
